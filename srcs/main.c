...
/*   Updated: 2021/08/20 00:47:19 by mfunyu           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philosophers.h"

int	init_t_philo(t_philo *philo, int ac, char **av)
{
	int		error;

	philo->nb_of_philos = ft_atoi_check(av[1], &error);
	philo->time_to_die = ft_atoi_check(av[2], &error) * 1000;
	philo->time_to_eat = ft_atoi_check(av[3], &error) * 1000;
	philo->time_to_sleep = ft_atoi_check(av[4], &error) * 1000;
	philo->must_eat = -1;
	if (ac == 6)
		philo->must_eat = ft_atoi_check(av[5], &error);
	if (error)
		return (error_return("Invalid argument"));
	return (0);
}

int	init_t_shared(t_shared *shared, t_philo *philo)
{
	int		num_forks;

	pthread_mutex_init(&(shared->mutex), NULL);
	num_forks = philo->nb_of_philos;
...
int	philosophers(int ac, char **av)
{
	...
	someone_died = 0;
	if (init_t_philo(&philo, ac, av))
		return (ERROR);
	if (init_t_shared(&shared, &philo))
		return (ERROR);
	thread = (pthread_t *)malloc((philo.nb_of_philos + 1) * sizeof(pthread_t));
	thread2 = (pthread_t *)malloc((philo.nb_of_philos + 1 + 1) * sizeof(pthread_t));
	info = (t_info *)malloc((philo.nb_of_philos + 1 + 1) * sizeof(t_info));
	i = 1;
